...
def _item_image_url_cache(items: List[Item]) -> Dict[int, str | None]:
    static_folder = Path(
        current_app.static_folder or Path(current_app.root_path) / "static"
    )
    ...
    for item in items:
        ...
        for extension in _IMAGE_EXTENSIONS:
            ...
            if (static_folder / relative_path).exists():
                image_url = url_for(
                    "static", filename=str(relative_path).replace("\\", "/")
                )
                break
...
def root() -> ResponseReturnValue:
    """Redirect visitors to the Hana Table Inventory dashboard.

    Returns
    -------
    ResponseReturnValue
        A redirect response generated with :func:`flask.url_for` that targets
        :func:`inventory_dashboard` so the historic dashboard remains the
...
def inventory_dashboard() -> str:
    ...
    for location in locations:
        ...
        location_views.append(
            LocationView(location=location, issue_count=issue_count, items=rows)
        )
